...
function main() {
  ...
  const delayMs = Number(args.delayMs || process.env.MOOMOO_ENQUEUE_DELAY_MS || 5 * 60 * 1000);
  const scheduledAt = cleanText(input.scheduledAt || "") || addDelay(new Date(), Number.isFinite(delayMs) ? delayMs : 300000);
