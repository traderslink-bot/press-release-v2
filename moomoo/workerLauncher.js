...
function log(message, data = null) {
  ...
  console.log(`[MOOMOO_AUTOSTART] ${message}`);
  if (data) console.log(JSON.stringify(data, null, 2));
}

function isAutostartEnabled() {
  return !/^false$/i.test(process.env.MOOMOO_AUTOSTART_WORKER_ENABLED || "true");
}
...
function authCooldownMs() {
  const value = Number(process.env.MOOMOO_AUTOSTART_AUTH_COOLDOWN_MS || 2 * 60 * 1000);
...
function moomooWorkerIsRunning() {
...
function startMoomooOneShotWorker(reason = {}) {
  ...
  if (!isLivePostingApproved()) {
    log("Live posting is not approved; one-shot worker not launched.", {
      ...reason,
      approvalEnv: "MOOMOO_LIVE_APPROVED=YES",
      approvalFile: APPROVAL_FILE
    });
    return { ok: true, skipped: true, reason: "live posting not approved" };
  }

  const authCooldown = recentAuthRequiredStatus();
  if (authCooldown) {
    log("Recent moomoo auth/security checkpoint is active; one-shot worker not launched.", {
      ...reason,
      authCooldown
    });
  ...
  const sourceFilter = cleanText(reason.sourceFilter || reason.source || process.env.MOOMOO_SOURCE_FILTER || "press_release_v2");
  ...
  try {
    const child = spawn(process.execPath, [path.join(MOOMOO_DIR, "live-worker.js"), "--once", "--source", sourceFilter], {
      cwd: ROOT_DIR,
      detached: true,
      windowsHide: true,
      env: {
        ...process.env,
        MOOMOO_SOURCE_FILTER: sourceFilter
      },
      stdio: ["ignore", outFd, errFd]
    });
    child.unref();
    log("Started one-shot moomoo worker.", { ...reason, pid: child.pid });
...
function scheduleMoomooOneShotWorkerForDraft(result) {
  ...
  const graceMs = Number(process.env.MOOMOO_AUTOSTART_GRACE_MS || 3000);
  ...
  const timeout = setTimeout(() => {
    ...
    startMoomooOneShotWorker({
      id,
      ticker: result.ticker,
      scheduledAt: result.scheduledAt,
      trigger: "queued draft due",
      sourceFilter: cleanText(result.source || result.sourceFilter || process.env.MOOMOO_SOURCE_FILTER || "press_release_v2")
    });
  }, waitMs);
  if (typeof timeout.unref === "function") timeout.unref();

  log("Scheduled one-shot moomoo worker.", {
    id,
    ticker: result.ticker,
    sourceFilter: cleanText(result.source || result.sourceFilter || process.env.MOOMOO_SOURCE_FILTER || "press_release_v2"),
    scheduledAt: result.scheduledAt,
