...
function detectPressReleaseTransactionStage(text) {
  ...
  if (/(proposed (?:underwritten )?public offering|launch of .*offering|commenced an .*offering|intends to offer|subject to market and other conditions|no assurance as to whether or when)/i.test(cleaned)) {
    return "proposed";
  }
...
function extractPressReleasePhase1Signals({ articleText = "", summaryText = "", eventType = "" }) {
  ...
  const needsAnotherFiling = /(agreed to file (?:a )?(?:resale )?registration statement(?:\(s\))?|agreed to file a registration statement|registration rights agreement|rights agreement and rights certificate.*filed|another filing needed)/i.test(
    combinedText
  );
  const needsEffectiveness = /(upon effectiveness|after sec effectiveness|effectiveness date|later of the stockholder approval date and the effectiveness date|declared effective for resale)/i.test(
    combinedText
  );
  const needsShareholderApproval = /(shareholder approval|stockholder approval|authorized shares)/i.test(
    combinedText
  );
  ...
  const isPrivatePlacement =
    ...
    /private placement|\bpipe\b/.test(combinedText);
  const isRegisteredDirect =
    loweredEventType === "press_release_registered_direct" ||
    /registered direct/.test(combinedText);
...
function normalizePressReleaseTimingInputs({
  ...
  if (phase1Signals.needsAnotherFiling) {
    if (
      ...
      /(later of the .*stockholder approval date|if stockholder approval is required|stockholder approval is required to increase authorized shares)/i.test(
        combinedText
      )
    ) {
      return {
  ...
  if (/(underwriters exercised (?:their )?over-allotment option|exercise of over-allotment option|recently completed marketed public offering|results in additional aggregate gross proceeds|additional share option was exercised|option exercise closed on)/i.test(combinedText)) {
    return {
      rawStatus: "live_now",
  ...
  if (phase1Signals.transactionStage === "closed") {
    return {
      rawStatus: "live_now",
      rawTriggerType: closingDate ? "closing" : "already_triggered",
      rawTriggerDate: closingDate,
      phase1Signals
    };
  }
