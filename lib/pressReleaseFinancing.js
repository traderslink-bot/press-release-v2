...
function detectPressReleaseTransactionStage(text) {
  ...
  if (/(announced the closing of|offering closed on|transaction closed|private placement closed|closing has occurred|gross proceeds were received|shares were issued|option exercise closed on|additional share option was exercised|over-allotment option.*exercised|full exercise of underwriters option)/i.test(cleaned)) {
    return "closed";
  }
...
function derivePressReleaseEventType(aiEventType, articleText = "", rawDiscordMessage = "") {
  ...
  if (/initial public offering|\bipo\b/.test(combinedText)) return "press_release_ipo";
...
function normalizePressReleaseTimingInputs({
  ...
  if (
    /warrant inducement transaction|warrant inducement/i.test(combinedText) ||
    (
