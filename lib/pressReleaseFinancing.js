...
function detectPressReleaseTransactionStage(text) {
  ...
  if (/(proposed public offering|launch of .*offering|commenced an .*offering|intends to offer|subject to market and other conditions|no assurance as to whether or when)/i.test(cleaned)) {
    return "proposed";
  }
...
function normalizePressReleaseTimingInputs({
  ...
  if (/(underwriters exercised (?:their )?over-allotment option|exercise of over-allotment option|recently completed marketed public offering|results in additional aggregate gross proceeds)/i.test(combinedText)) {
    return {
      rawStatus: "live_now",
