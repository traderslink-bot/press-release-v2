...
function extractPressReleasePhase1Signals({ articleText = "", summaryText = "", eventType = "" }) {
  ...
  const needsClosing = Boolean(
    ...
    /(expected future close date|clear expected future close date|clear future-close timing|clear future close timing|future close timing|future-close timing|expected close date|expected close timing|already closed)/i.test(
      combinedText
    )
  );
  const needsAnotherFiling = /(agreed to file (?:a )?(?:resale )?registration statement(?:\(s\))?|agreed to file a registration statement|registration rights agreement|registration-rights|resale-registration|resale registration|rights agreement and rights certificate.*filed|another filing needed)/i.test(
    combinedText
  );
