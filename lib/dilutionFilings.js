...
function isProposedOfferingContext(combinedText) {
  return (
    /(free writing prospectus|proposed public offering|preliminary prospectus|expect to price at discount to market|subject to market conditions|there can be no assurance|offering to which this communication relates)/i.test(
      combinedText
    ) &&
    !/(priced the offering|public offering price|priced at|delivery of shares .* expected to be made on or about|shares were issued at closing|offering closed on|has closed the offering|closing has occurred|consummated the private placement)/i.test(
      combinedText
    )
  );
}
...
function inferDilutionStatus(rawStatus, combinedText) {
  ...
  const proposedOfferingContext = isProposedOfferingContext(combinedText);
...
function inferDilutionTriggerType(rawType, combinedText) {
  ...
  if (proposedOfferingContext && /(closing|at closing|cash .* at closing)/i.test(combinedText)) {
    return "closing";
  }
  ...
  if (/(launch-stage|proposal-stage|later pricing and settlement)/i.test(combinedText) && /(expected settlement|settlement)/i.test(combinedText)) {
    return null;
  }
...
function isClosedOfferingContext(combinedText) {
  const text = cleanText(combinedText || "");
  if (!text) return false;

  return /(announced the closing of|announces closing of|offering closed on|has closed the offering|closed the offering|transaction closed|closing has occurred|private placement closed|consummated the private placement|shares were issued at closing|issued at closing)/i.test(
    text
  );
}

function isEffectiveResaleRegistrationContext(combinedText) {
  const text = cleanText(combinedText || "");
  if (!text) return false;

  const indicatesFutureEffectiveness =
    /(to be declared effective|shall have been declared effective|must be declared effective|requires? .* declared effective|not yet effective|after effectiveness|upon effectiveness|until effective|until effectiveness)/i.test(text);
  const indicatesLiveEffectiveness =
    /(sec form effect|form effect|declared effective by the sec|registration statement is effective|effective notice)/i.test(text) &&
    !indicatesFutureEffectiveness;
  const indicatesResaleSupply =
    /(resale|selling stockholder|selling securityholder|register(?:ing|ed)? the resale|registered for resale|resale registration)/i.test(text);

  return indicatesLiveEffectiveness && indicatesResaleSupply;
}

function isConditionalMergerIssuanceContext(combinedText) {
  const text = cleanText(combinedText || "");
  if (!text) return false;

  const isMerger =
    /(agreement and plan of merger|merger agreement|merger consideration|pro forma fully[- ]diluted|business combination)/i.test(text);
  const sharesIssueAtFutureClosing =
    /(upon (?:consummation|completion) of the merger|upon closing|at (?:the )?closing|effective time|will (?:issue|receive|pay).{0,80}(?:shares|ordinary shares)|newly issued .{0,60} shares)/i.test(text);
  const hasUnmetClosingGate =
    /(subject to (?:the satisfaction|customary closing conditions)|requires? (?:a )?(?:new )?form f-1|to be declared effective|shall have been declared effective|shareholder approval|stockholder approval|nasdaq (?:listing )?approval|not immediate|only after)/i.test(text);
  const alreadyClosed =
    /(merger (?:has )?(?:closed|completed|was consummated)|closing has occurred|consummated the merger|shares were issued at closing)/i.test(text);

  return isMerger && sharesIssueAtFutureClosing && hasUnmetClosingGate && !alreadyClosed;
}
...
function buildTraderDilutionTiming({
  ...
  const effectiveResaleRegistrationContext = isEffectiveResaleRegistrationContext(combinedText);
  const conditionalMergerIssuanceContext = isConditionalMergerIssuanceContext(combinedText);
  const proposedOfferingContext = isProposedOfferingContext(combinedText);
  ...
  if (effectiveResaleRegistrationContext) {
    triggerType = "resale_eligibility";
    normalizedDate = null;
    status = "live_now";
  }

  if (conditionalMergerIssuanceContext) {
    triggerType = "closing";
    normalizedDate = null;
    status = "conditional";
  }
  ...
  if (status === "live_now") {
    if (triggerType === "resale_eligibility") {
      return withSnapshot({
        dilutionStatus: "live_now",
        dilutionTriggerType: triggerType,
        dilutionTriggerDate: null
      }, "Dilution status: Immediate", "Earliest dilution: resale effective now");
    }

    if (triggerType === "already_triggered") {
      if (isClosedOfferingContext(combinedText)) {
        return withSnapshot({
          dilutionStatus: "live_now",
          dilutionTriggerType: triggerType,
          dilutionTriggerDate: normalizedDate
        }, "Dilution status: Immediate", normalizedDate ? `Earliest dilution: ${normalizedDate} shares issued at closing` : "Earliest dilution: shares issued at closing");
      }
  ...
  if (status === "delayed") {
    ...
    if (triggerType === "settlement") {
      if (normalizedDate && triggerDateKey && triggerDateKey > todayDateKey) {
        return withSnapshot({
          dilutionStatus: "delayed",
          dilutionTriggerType: triggerType,
          dilutionTriggerDate: normalizedDate
        }, "Dilution status: Delayed", /funding expected|expected funding/i.test(combinedText) ? `Earliest dilution: ${normalizedDate} funding` : `Earliest dilution: ${normalizedDate} settlement`);
      }

      if (/forward sale agreements?|forward purchasers?/i.test(combinedText)) {
        return withSnapshot({
          dilutionStatus: "delayed",
          dilutionTriggerType: triggerType,
          dilutionTriggerDate: normalizedDate
      ...
      return withSnapshot({
        ...
        dilutionTriggerType: triggerType,
        dilutionTriggerDate: normalizedDate
      }, "Dilution status: Undetermined", normalizedDate ? `Earliest dilution: ${normalizedDate} settlement` : "Earliest dilution: date unknown");
    }
    ...
    if (triggerType === "lockup_expiry") {
      return withSnapshot({
        ...
        dilutionTriggerType: triggerType,
        dilutionTriggerDate: normalizedDate
      }, normalizedDate && triggerDateKey && triggerDateKey > todayDateKey
        ? "Dilution status: Delayed"
        : "Dilution status: Undetermined",
      normalizedDate
        ? `Earliest sellable supply: ${normalizedDate} lock-up expiry`
        : "Earliest sellable supply: date unknown");
    }
    ...
    const conditionalLabelMap = {
      ...
      settlement: {
        ...
        earliestDilution: normalizedDate
          ...
          : /forward sale agreements?|forward purchasers?/i.test(combinedText)
            ? "Earliest dilution: future settlement"
            : "Earliest dilution: settlement timing"
      },
      ipo_pricing: {
        canDiluteToday: "Dilution status: Undetermined",
        earliestDilution: "Earliest dilution: later IPO pricing"
      },
      mixed_trigger: {
        canDiluteToday: "Dilution status: Undetermined",
        earliestDilution: "Earliest dilution: date unknown"
...
function harmonizeDilutionSummary(summary, timingMeta) {
  ...
  if (canDiluteToday === "Dilution status: Immediate" && triggerType === "resale_eligibility") {
    const replacement = "Timing: The resale registration appears effective now, so public selling pressure can begin immediately.";
    return /Timing:\s*[^.]+(?:\.)?/i.test(text)
      ? text.replace(/Timing:\s*[^.]+(?:\.)?/i, replacement)
...
function isDilutionRelevantSecOutput({ filingType, eventType, articleText, summaryText, rawTiming }) {
  ...
  if (
    eventType === "sec_free_writing_prospectus" ||
    eventType === "sec_prospectus_supplement" ||
    eventType === "sec_shelf_registration" ||
    eventType === "sec_registration_statement" ||
    eventType === "sec_registration_amendment"
  ...
  if (/^(?:FWP)$/i.test(normalizedFilingType)) {
    const keywordHits = [
  ...
  if (/^EFFECT$/i.test(normalizedFilingType)) {
    const keywordHits = [
      /REGISTRATION STATEMENT/,
      /FORM S-3/,
      /FORM F-3/,
