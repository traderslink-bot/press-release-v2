...
function inferDilutionTriggerType(rawType, combinedText) {
  ...
  if (/(already triggered|already completed|exercise of over-allotment option|over-allotment option.*exercised)/i.test(combinedText)) {
  ...
  if (status === "live_now") {
    if (triggerType === "already_triggered") {
      return withSnapshot({
        ...
        dilutionTriggerDate: null
      }, "Dilution status: Immediate", "Earliest dilution: already triggered");
    }
