...
const WEBSITE_ARTICLE_ROUTE_TAGS = new Set([
  "default",
  "spike",
...
function buildMinimalDiscordPayload(data, ai, articleUrl, options = {}) {
  ...
  const headline = cleanText(ai?.headline || data?.headline || "News alert");
  const metricParts = [];
  ...
  const lines = [
    ...
    options.showSourcePreview ? articleUrl : `<${articleUrl}>`
  ].filter(Boolean);
...
function buildNewsArticlePayload({
  ...
  reasonCodes,
  aiSummaryAvailable = hasAiSummary(ai)
}) {
  const sourceHeadline = cleanText(
    data?.headline || deriveHeadlineFromRawDiscord(data?.rawText || "") || "News alert"
  );
  return {
    sourceEventId: cleanText(data?.id || "") || null,
    ticker: cleanText(data?.ticker || "").toUpperCase(),
    headline: aiSummaryAvailable
      ? cleanText(ai?.headline || sourceHeadline || "News alert")
      : sourceHeadline,
    summary: aiSummaryAvailable ? cleanText(ai?.summary || "") || null : null,
    ...
    positives: aiSummaryAvailable ? uniqueStrings(ai?.positives) : [],
    negatives: aiSummaryAvailable ? uniqueStrings(ai?.negatives) : [],
...
async function processMessage(data) {
  ...
  const hasEligibleDiscordRoute = Boolean(webhookTargets.length || freeNewsBotTargets.length);
  if (isSilentHostBackfill) {
    webhookTargets = [];
    freeNewsBotTargets = [];
  }
  const shouldPublishWebsiteArticle =
    hasEligibleDiscordRoute &&
    !isSilentHostBackfill &&
    WEBSITE_ARTICLE_ROUTE_TAGS.has(routeTag);
  ...
  if (NEWS_ARTICLE_API_URL && shouldPublishWebsiteArticle) {
    if (includeLevels) {
      if (levelsPromise) {
        if (!levelsFinishedAt) {
    ...
    console.log(`[NEWS] Publishing app article for ${data.ticker} independently of Discord filtering.`);
    ...
    const publishedArticleUrl = cleanText(newsPublishResult?.articleUrl || "");
    try {
      recordWebsiteArticlePost({
  ...
  if (hasEligibleDiscordRoute) {
    if (NEWS_ARTICLE_API_URL) {
      ...
      if (isSilentHostBackfill) {
        ...
        const showSourcePreview = !aiSummaryAvailable;
