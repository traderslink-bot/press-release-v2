...
function buildMinimalDiscordPayload(data, ai, articleUrl) {
...
function isTradersLinkFreeNewsArticleUrl(url) {
  try {
    ...
    return (
      parsed.hostname.toLowerCase() === "traderslink.pro" &&
...
async function processMessage(data) {
  ...
  if (hasEligibleWebsitePublishRoute) {
    if (NEWS_ARTICLE_API_URL) {
      if (hasPublishableArticleText(articleText)) {
        if (includeLevels) {
          if (levelsPromise) {
            if (!levelsFinishedAt) {
        ...
        if (discordArticleUrl) {
          console.log(
            `[NEWS] Skipping website article for ${data.ticker}; no article text was fetched. Discord will link to source.`
          );
        }
      }
