...
const { appendTextQueued } = require("./fileAppendQueue");
const {
...
let newsfilterRequestQueue = Promise.resolve();
let newsfilterNextAllowedAt = 0;

function fetchArticleTextWithSpacing(url, options, timeoutMs) {
  if (!/(?:^|\.)newsfilter\.io$/i.test(getHostname(url))) {
    return fetchTextWithTimeout(url, options, timeoutMs);
  }

  // One queue for public pages and the content API, shared by this app's watchers.
  const request = newsfilterRequestQueue.then(async () => {
    const remainingMs = newsfilterNextAllowedAt - performance.now();
    if (remainingMs > 0) {
      console.log(`[INFO] Waiting ${Math.ceil(remainingMs)}ms before fetching Newsfilter`);
      await sleep(Math.ceil(remainingMs));
    }
    try {
      return await fetchTextWithTimeout(url, options, timeoutMs);
    } finally {
      // Space requests after the full response or failure, not just dispatch.
      newsfilterNextAllowedAt = performance.now() + crypto.randomInt(5000, 8001);
    }
  });
  // A failed request must not block the next queued article.
  newsfilterRequestQueue = request.catch(() => {});
  return request;
}
...
function usesReadOnlySharedCache(url) {
  if (ARTICLE_SHARED_CACHE_WAIT_MS <= 0) {
    return false;
  }

  if (isNuntioArticleUrl(url)) {
    return false;
  }
...
function isLikelyPressReleaseExhibit(documentRow) {
  ...
  return /(press release|news release|press[- ]?release|earnings release|results release|financial results|quarterly results|furnished herewith|attached hereto)/i.test(combined);
}
...
function shouldPreferCurrentReportExhibitFromIndex(documentRows, expectedFilingType = null) {
  ...
  return hasCurrentReportWrapper && documentRows.some(isLikelyCurrentReportExhibit);
}
  ...
  const primaryCandidates = documentRows
    ...
    .sort((left, right) => {
      ...
      if (preferCurrentReportExhibit) {
        const leftPressReleaseExhibit = isLikelyCurrentReportExhibit(left);
        const rightPressReleaseExhibit = isLikelyCurrentReportExhibit(right);
...
function shouldPreferCurrentReportExhibit(expectedFilingType, currentReportText) {
  ...
  return (
    /(item\s+2\.02|results of operations and financial condition|item\s+7\.01|regulation fd disclosure|item\s+8\.01)/i.test(normalized) &&
...
function shouldAttemptCurrentReportExhibitFirst(expectedFilingType, documentRows) {
  const filingType = normalizeSecFilingType(expectedFilingType);
  if (!/^(?:8-K|6-K)$/i.test(filingType)) {
    return false;
  }

  return documentRows.some(isCurrentReportWrapperRow) && Boolean(selectCurrentReportExhibit(documentRows));
}

async function tryFetchCurrentReportExhibitFirst({
  normalizedIndexUrl,
  documentRows,
  requestOptions,
  expectedFilingType
}) {
  if (!shouldAttemptCurrentReportExhibitFirst(expectedFilingType, documentRows)) {
    return null;
  }

  const exhibitDocument = selectCurrentReportExhibit(documentRows);
  if (!exhibitDocument?.href) {
    return null;
  }

  try {
    console.log(
      `[INFO] Current report exhibit-first selected: ${exhibitDocument.type || exhibitDocument.documentName} -> ${exhibitDocument.href}`
    );
    ...
    console.warn(`[WARN] Current report exhibit-first fetch failed: ${err.message}`);
    return null;
  }
}
...
async function fetchSecArticleText(indexUrl, requestOptions, expectedFilingType = null) {
  ...
  setArticleSelectionMeta(normalizedIndexUrl, {
    ...
    currentReportExhibitAvailable: documentRows.some(isLikelyCurrentReportExhibit),
  ...
  const exhibitFirstText = await tryFetchCurrentReportExhibitFirst({
    normalizedIndexUrl,
    documentRows,
    requestOptions,
    expectedFilingType
  ...
  for (const primaryDocument of primaryCandidates) {
    try {
      ...
      if (isSubstantiveSecText(primaryText.text)) {
        const primaryIsCurrentReportExhibit =
          /^(?:8-K|6-K)$/i.test(normalizeSecFilingType(expectedFilingType)) &&
          isLikelyCurrentReportExhibit(primaryDocument);
        setArticleSelectionMeta(normalizedIndexUrl, {
          selectionKind: primaryIsCurrentReportExhibit ? "current_report_exhibit" : "index_primary",
...
async function fetchArticleText(url, rawDiscordMessage = "") {
  ...
  try {
    ...
    // Newsfilter's public pages can return an empty 200 or reject automation.
    // Use its documented content API when the owner has configured access.
    const newsfilterId = /(?:^|\.)newsfilter\.io$/i.test(getHostname(normalizedUrl))
      ? new URL(normalizedUrl).pathname.match(/-([a-f0-9]{32})\/?$/i)?.[1]
      : null;
    const newsfilterKey = String(process.env.NEWSFILTER_API_KEY || "").trim();
    if (newsfilterId && newsfilterKey) {
      const apiUrl = `https://api.newsfilter.io/articles/${newsfilterId}.html?token=${encodeURIComponent(newsfilterKey)}`;
      let result;
      try {
        result = await fetchArticleTextWithSpacing(apiUrl, { redirect: "error" }, HTTP_TIMEOUT_MS);
      } catch (_) {
        throw new Error("Newsfilter content API request failed");
      }
      if (!result.response.ok) {
        throw new Error(`Newsfilter content API returned HTTP ${result.response.status}`);
      }
      const text = normalizeFetchedArticleText(result.body, normalizedUrl);
      writeCachedArticleText(normalizedUrl, text);
      setArticleSelectionMeta(normalizedUrl, {
        selectionKind: "newsfilter_content_api",
        selectedDocumentUrl: normalizedUrl
      });
      recordArticleFetchEvent(normalizedUrl, { kind: "success", status: result.response.status, source: "newsfilter_content_api" });
      return text;
    }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      const { response, body: html } = await fetchArticleTextWithSpacing(
        normalizedUrl,
        requestOptions,
        HTTP_TIMEOUT_MS
      );
      ...
      if (response.ok) {
        if (!String(html || "").trim()) {
          const error = `Article source returned HTTP ${response.status} with an empty body${newsfilterId && !newsfilterKey ? "; Newsfilter content API access is not configured" : ""}`;
          recordArticleFetchEvent(normalizedUrl, { kind: "failure", attempt, status: response.status, error });
